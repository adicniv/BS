...
class hikashopBridgePaginationHelper extends JPagination {
	...
	function _link($start){
		...
		if(isset($_GET['limitstart'])){
			...
			return str_replace(array('limitstart'.$this->hikaSuffix.'='.$old_start, 'limitstart'.$this->hikaSuffix.'-='.$old_start), array('limitstart'.$this->hikaSuffix.'='.$start, 'limitstart'.$this->hikaSuffix.'-'.$start), $current_url);
		}
		$sep = '?';
		if(strpos($current_url, '?'))
			$sep = '&';
		return $current_url.$sep.'limitstart'.$this->hikaSuffix.'='.$start;
	}
